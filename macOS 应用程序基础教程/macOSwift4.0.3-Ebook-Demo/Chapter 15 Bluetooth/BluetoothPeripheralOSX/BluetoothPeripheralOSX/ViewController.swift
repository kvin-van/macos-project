...
extension ViewController: CBPeripheralManagerDelegate {
    ...
    func peripheralManagerDidUpdateState(_ peripheral: CBPeripheralManager) {
        ...
        case .unauthorized:
            ...
            break
        case .unsupported:
            stateDesc = "The app is not authorized to use Bluetooth Low Energy 该应用程序未被授权使用蓝牙低能耗"
            break
        case .poweredOff:
            stateDesc = "Bluetooth is currently powered off 蓝牙当前已关闭"
            break
        case .poweredOn:
    ...
    func peripheralManager(_ peripheral: CBPeripheralManager, didAdd service: CBService, error: Error?) {
        guard error == nil else {
            print("添加服务时出错: \(error!.localizedDescription)")
            return
        }
    ...
    func peripheralManagerDidStartAdvertising(_ peripheral: CBPeripheralManager, error: Error?) {
        guard error == nil else {
            print("Error startAdvertising启动广告时出错: \(error!.localizedDescription)")
            return
        }
