...
//        self.gcdTimerTest()
    ...
    func gcdTimerTest(){
        ...
        //配置为重复执行的定时器
//        gcdTimer?.schedule(deadline: .now() + 1, repeating: 1)  //每隔一秒执行一次 重复
        gcdTimer?.schedule(deadline: .now() + 5)   //单次定时器  5秒执行
...
//        var observerContext = CFRunLoopObserverContext(version: 0, info: &_self, retain: nil, release: nil, copyDescription: nil)
        //2.
        var observerContext = CFRunLoopObserverContext(version: 0, info: UnsafeMutableRawPointer(Unmanaged.passUnretained(self).toOpaque()), retain: nil, release: nil, copyDescription: nil)
