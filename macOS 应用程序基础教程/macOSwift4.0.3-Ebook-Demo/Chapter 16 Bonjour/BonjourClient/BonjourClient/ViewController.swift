...
class ViewController: NSViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.serviceBrowser = NetServiceBrowser() //网络服务浏览器
...
extension ViewController: NetServiceBrowserDelegate {
    ...
    func netServiceBrowser(_ browser: NetServiceBrowser, didFind service: NetService, moreComing: Bool) {
        ...
        print("didFindService发现服务 \(service)")
    }
...
extension ViewController: NetServiceDelegate {
    ...
    func netServiceDidResolveAddress(_ sender: NetService) {
        //完成解析,取得服务的host和port
        print("netServiceDidResolveAddress hostName 已经解析的主机名 \(sender.hostName) domain \(sender.domain)  port \(sender.port) name \(sender.name) ")
