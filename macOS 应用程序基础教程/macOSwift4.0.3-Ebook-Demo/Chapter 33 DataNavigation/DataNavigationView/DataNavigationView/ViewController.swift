...
class ViewController: NSViewController {
    ...
    func dataNavigationItemsConfig() ->[DataNavigationItem] {
        ...
        insertItem.imageName =  NSImage.addTemplateName
        insertItem.tooltips = "insert a row into current table在当前表中插入一行"
        insertItem.tag = .add
        
        let deleteItem = DataNavigationButtonItem()
        deleteItem.imageName = NSImage.removeTemplateName
        deleteItem.tooltips = "delete seleted rows form current table"
        deleteItem.tag = .remove
        
        let refreshItem = DataNavigationButtonItem()
        refreshItem.imageName = NSImage.refreshTemplateName
